...
function LinkedInPreview({ post, article, user }) {
  const name = getDisplayName(user);
...
function TwitterPreview({ post, article, user }) {
  const name = getDisplayName(user);
...
export default function ContentAgent() {
  ...
  useEffect(() => {
    ...
    setStudioStory(articleFromRoute.description || '');
  ...
  const handleSendStudioToCreate = (schedule = false) => {
    if (!studioPost.trim()) {
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[280px_minmax(0,1fr)_280px] min-h-[620px]">
            ...
            <div className="p-4 flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 2xl:grid-cols-2 gap-4">
                    {selectedPlatforms.includes('linkedin') && (
                      <div>
                        ...
                        <LinkedInPreview post={studioPost} article={studioArticle} user={user} />
                      </div>
                    )}
                    ...
                    {selectedPlatforms.includes('twitter') && (
                      <div>
                        ...
                        <TwitterPreview post={studioPost} article={studioArticle} user={user} />
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex flex-wrap justify-end gap-2">
                <button
                  onClick={() => handleSendStudioToCreate(false)}
                ...
                <button
                  onClick={() => handleSendStudioToCreate(true)}
                  disabled={!studioPost.trim()}
                  className="px-4 py-2 bg-white/[0.06] border border-white/[0.08] text-white rounded-lg hover:bg-white/[0.1] transition-colors disabled:opacity-50 text-sm font-medium"
                >
                  Schedule
